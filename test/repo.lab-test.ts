...
{ all:
  ...
  { path: 'test-a/srv/options.js',
    ...
    //console.dir(found,{depth:null})
    expect(found).equals([ { path: 'test-b/.travis.yml',
    kind: 'yml',
    name: '.travis.yml',
    text: 'travis-B' },
  ...
  it('intern.render_template', () => {
    ...
    var out = Repo.intern.render_template(tm0, {name:'tm0',props:{qaz:'wsx'}})
    //console.log(out)
...
`
    ...
    var out =
      Repo.intern.render_template(tm1, {name:'tm01',props:{qaz:'wsx'}}, text1_src)
    //console.log(out)
...
`
            ...
            templates: {
              ...
              ccc: {
                ...
                eee: 'A <%=name%> B <%=props.x%> C <%=props.y%>'+
                  ' D <%=slots.zed%> E'
              }
            }
          }
    ...
    for(var repo of repos) {
      ...
      for(var file of files) {
        content[repo][file] = Fs.readFileSync('work/'+repo+'/'+file).toString()
      }
    }

    // console.dir(content)

    expect(content).equal({ foo:
   { aaa: 'lorem',
     bbb: 'ipsum foo',
     fff: 'QQQ0',
     'ggg/hhh': 'WWW0',
     ...
     'ccc/eee': 'A foo B 1 C 3 D  foozed  E' },
  bar:
   { aaa: 'lorem',
     bbb: 'ipsum bar',
     fff: 'QQQ1',
     ...
     'ccc/eee': 'A bar B 2 C  D  barzed  E' } })
    
    /*
    //console.log('aaa', Fs.readFileSync('work/foo/aaa'))
    console.log('QQQ0', Fs.readFileSync('work/foo/fff').toString())
    console.log('QQQ1', Fs.readFileSync('work/bar/fff').toString())

    console.log('foo aaa :', Fs.readFileSync('work/foo/aaa').toString())
    console.log('bar aaa :', Fs.readFileSync('work/bar/aaa').toString())

    console.log('foo aaa :', Fs.readFileSync('work/foo/aaa').toString())
    console.log('bar aaa :', Fs.readFileSync('work/bar/aaa').toString())

    console.log('foo bbb :', Fs.readFileSync('work/foo/bbb').toString())
    console.log('bar bbb :', Fs.readFileSync('work/bar/bbb').toString())

    console.log('foo ccc/ddd :', Fs.readFileSync('work/foo/ccc/ddd').toString())
    console.log('bar ccc/ddd :', Fs.readFileSync('work/bar/ccc/ddd').toString())

    console.log('foo ccc/eee :', Fs.readFileSync('work/foo/ccc/eee').toString())
    console.log('bar ccc/eee :', Fs.readFileSync('work/bar/ccc/eee').toString())
    */
  })
})
