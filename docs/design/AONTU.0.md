...
jostraca. A pipe is the whole integration, and that it is enough is
itself a finding: aontu's G9 D3 has aontu importing jostraca as an
...
its language server away from the file writer. None of that is needed to
run the pipeline. It is needed for `aontu render` to write files itself,
...
`a ZZZ b`. aontu's G9 §5 asks for a `raw: true` prop on this side; the
tool passes an empty model instead, which is that note's own stated
workaround and is not a fix — a caller who wants a model for their own
components loses the protection. `raw: true` is the ask, and it is
small.
