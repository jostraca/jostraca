...
function fserr(code: string, syscall: string, path: string, dest?: string) {
  const where = null == dest ? `'${path}'` : `'${path}' -> '${dest}'`
  const err: any = new Error(`${code}: ${ERRMSG[code]}, ${syscall} ${where}`)
  err.code = code
  err.syscall = syscall
  err.path = path
  if (null != dest) { err.dest = dest }
  return err
}
...
function memClean(p: string, cwd?: string): string {
  let s = String(p).replace(/\\/g, '/')

  const isDrive = (v: string) => /^[A-Za-z]:\//.test(v)
...
function parentOf(cp: string): string {
...
class MemVolume {
  ...
  touch(cp: string) {
    if (!this.seq.has(cp)) { this.seq.set(cp, this.nextSeq++) }
  }
  ...
  children(cp: string): string[] {
    ...
    const collect = (key: string) => {
      if (!key.startsWith(prefix) || key === cp) { return }
  ...
  toJSON() {
    ...
    const add = (full: string) => {
      ...
      if (null == list) { kids.set(parent, [full]) }
      else { list.push(full) }
    }
    this.files.forEach((_v, k) => add(k))
    this.dirs.forEach((_v, k) => { if ('/' !== k) { add(k) } })
...
function resolveLink(vol: MemVolume, cp: string): string {
  let cur = cp
  for (let depth = 0; depth < 32; depth++) {
    const target = vol.links.get(cur)
    if (null == target) { return cur }
    cur = memClean(target)
  }
  return cur
}
...
function makeFs(vol: MemVolume) {
  const readEncoding = (opts?: any): string | null => {
    if (null == opts) { return null }
    if ('string' === typeof opts) { return opts }
    return null == opts.encoding ? null : opts.encoding
  }

  const statOf = (cp: string): Stats | undefined => {
    const target = resolveLink(vol, cp)
    const buf = vol.files.get(target)
    if (null != buf) {
      return new Stats('file', buf.length,
        S_IFREG | (vol.modes.get(target) ?? DEFAULT_FILE_MODE),
        vol.times.get(target) ?? 0)
    }
    if (vol.dirs.has(target)) {
      return new Stats('dir', 0,
        S_IFDIR | (vol.dirs.get(target) ?? DEFAULT_DIR_MODE),
        vol.times.get(target) ?? 0)
    }
    return undefined
  }
  ...
  const fs = {
    ...
    readFileSync(p: string, opts?: any): any {
      ...
      if (null == buf) {
        if (vol.dirs.has(cp)) { throw fserr('EISDIR', 'open', p) }
        throw fserr('ENOENT', 'open', p)
      }
      const enc = readEncoding(opts)
      return null == enc ? Buffer.from(buf) : buf.toString(enc as BufferEncoding)
    },
    ...
    writeFileSync(p: string, data: any, opts?: any): void {
      ...
      if (!vol.dirs.has(parent)) { throw fserr('ENOENT', 'open', parent) }

      const enc = readEncoding(opts)
      const buf = Buffer.isBuffer(data) ? Buffer.from(data) :
        Buffer.from(String(data), (enc || 'utf8') as BufferEncoding)
      ...
      if (null != mode) { vol.modes.set(cp, mode & 0o7777) }
      else if (!vol.modes.has(cp)) { vol.modes.set(cp, DEFAULT_FILE_MODE) }
    },
    ...
    mkdirSync(p: string, opts?: any): void {
      ...
      if (null != mode) { vol.dirs.set(cp, mode & 0o7777) }
    },
    ...
    statSync(p: string, opts?: any): any {
      ...
      if (undefined === st) {
        if (null != opts && false === opts.throwIfNoEntry) { return undefined }
    ...
    readdirSync(p: string): string[] {
      const cp = resolveLink(vol, memClean(p))
      if (!vol.dirs.has(cp)) {
        if (vol.files.has(cp)) { throw fserr('ENOTDIR', 'scandir', p) }
    ...
    renameSync(from: string, to: string): void {
      ...
      if (vol.files.has(cf)) {
        const buf = vol.files.get(cf) as Buffer
        ...
        if (null != mode) { vol.modes.set(ct, mode) }
        vol.times.set(ct, null == time ? Date.now() : time)
        if (!destExisted) { vol.retouch(ct) }
        return
      }

      if (vol.dirs.has(cf)) {
        // Re-key the subtree under the new prefix.
        const move = (m: Map<string, any>) => {
        ...
        move(vol.files); move(vol.times); move(vol.modes); move(vol.dirs); move(vol.links)
        move(vol.seq)
        return
      }
    ...
    unlinkSync(p: string): void {
      const cp = memClean(p)
      if (vol.links.delete(cp)) { return }
      if (!vol.files.has(cp)) {
        if (vol.dirs.has(cp)) { throw fserr('EISDIR', 'unlink', p) }
    ...
    chmodSync(p: string, mode: number): void {
      const cp = resolveLink(vol, memClean(p))
      if (vol.files.has(cp)) { vol.modes.set(cp, mode & 0o7777); return }
      if (vol.dirs.has(cp)) { vol.dirs.set(cp, mode & 0o7777); return }
    ...
    realpathSync(p: string): string {
      const cp = memClean(p)
      const target = resolveLink(vol, cp)
      if (undefined === statOf(target)) { throw fserr('ENOENT', 'realpath', p) }
      return target
    },
