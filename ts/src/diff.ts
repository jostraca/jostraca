...
//
// This is jostraca's own implementation, replacing the `node-diff3` and
// `diff` dependencies. Two reasons:
//
//  1. Performance. The old dependency was unusable on the workload this
//     project actually has — regenerating a large file whose lines mostly
//     repeat (config and reference output). Measured on identical inputs:
//     6.4 s at 5 000 lines, 62 s at 10 000, roughly ten minutes at 20 000.
//
//  2. Parity. The Go port hand-implemented its own merge, and the two
//     disagreed on ~72% of non-trivial merges — both producing valid but
//     different output. The only way two stacks stay byte-identical is to
//     run the same algorithm, so `go/diff.go` mirrors this file directly.
//
// Algorithm:
//
...
//     almost nothing.
//   - Run Hirschberg's algorithm on the remainder: O(N·M) time as before,
//     but O(M) space rather than a full table, where M is the length of
//     the SECOND argument — the two DP rows are sized by `b`.
//
//     Not O(min(N,M)): nothing swaps the inputs so the shorter side backs
//     the rows, and it must not. The split tie-break below is
//     order-sensitive, so swapping `a` and `b` changes merge output on
//     ~11% of random inputs. Reducing the constant (a narrower row
//     element type) is the output-preserving option if this ever matters.
...
// Explicit labels for the conflict markers. Overrides `when`/`last`.
type DiffLabels = {
  generated?: string
  existing?: string
}
...
type DiffSpec = {
  ...
  kind?: string
  // Full label override; wins over when/last/kind.
  labels?: DiffLabels
}

// Why a merge produced what it did. These map straight onto jostraca's
// audit breadcrumbs, which is the point: the caller does not have to
// re-derive the decision the merge already made.
//
//   same       - the file on disk already equals the new generate
//   clean      - the file on disk is untouched since the last generate
//   unresolved - the file on disk still holds unresolved conflict markers
//   merged     - a real three-way merge ran
...
// same    - generated and existing are identical
// changed - the two differ and the result is annotated
...
function hirschberg(a: string[], b: string[], out: string[]): void {
  if (0 === a.length || 0 === b.length) {
    return
  }
...
function lcsRow(a: string[], b: string[], reverse: boolean): number[] {
  let prev = new Array<number>(b.length + 1).fill(0)
  let cur = new Array<number>(b.length + 1).fill(0)

  const at = (xs: string[], i: number) => reverse ? xs[xs.length - 1 - i] : xs[i]
...
function alignLcs(base: string[], target: string[]): number[] {
...
function hunks(generated: string[], existing: string[]): Hunk[] {
...
function diff(generated: string, existing: string, spec?: DiffSpec): DiffResult {
  if (generated === existing) {
  ...
  const block = (block_lines: string[], label?: string) => {
