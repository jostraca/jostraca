...
function rows(): Row[] {
  ...
  text.split('\n').forEach((raw, i) => {
    ...
    Assert.equal(cols.length, 5,
      `docs/cmp-surface.tsv:${i + 1} wants 5 tab-separated columns, ` +
      `got ${cols.length}: ${line}`)
    if (!header) {
      Assert.deepEqual(cols, ['cmp', 'prop', 'type', 'required', 'note'],
        'docs/cmp-surface.tsv: unexpected header row')
      header = true
      return
    }
    out.push({
  ...
  Assert.ok(30 < out.length,
    `docs/cmp-surface.tsv: only ${out.length} rows loaded`)
  return out
}
...
describe('cmp-surface', () => {

  // 1. The components are the ones on disk.
  test('every-component-has-rows', () => {
  ...
  test('every-declared-prop-is-read-and-every-read-prop-is-declared', () => {
    ...
    for (const [cmp, cmprows] of byCmp) {
      ...
      Assert.equal(stars.length, 1,
        `docs/cmp-surface.tsv: ${cmp} wants exactly one \`*\` row, ` +
        `got ${stars.length}`)
      ...
      Assert.deepEqual(names, [...new Set(names)],
        `docs/cmp-surface.tsv: ${cmp} declares a prop twice`)

      Assert.deepEqual(
        names.filter((n) => !unused.includes(n)), read,
        `docs/cmp-surface.tsv and src/cmp/${cmp}.ts disagree about props ` +
        `(a prop marked \`unused:\` is exempt)`)

      for (const prop of unused) {
        Assert.ok(!read.includes(prop),
          `docs/cmp-surface.tsv: ${cmp}.${prop} is marked \`unused:\` ` +
          `and src/cmp/${cmp}.ts reads it`)
      }

      for (const row of declared) {
        Assert.match(row.required, /^(yes|no)$/,
          `docs/cmp-surface.tsv:${row.line} required is yes or no`)
        Assert.ok('' !== row.type && '-' !== row.type,
          `docs/cmp-surface.tsv:${row.line} needs a type`)
        Assert.ok('' !== row.note,
          `docs/cmp-surface.tsv:${row.line} needs a note`)
      }
    }
  })
  ...
  test('an-unknown-prop-does-what-the-star-row-says', async () => {
    const stars = rows().filter((r) => '*' === r.prop)
    Assert.equal(stars.length, Object.keys(TREE_CMP).length)

    for (const row of stars) {
      Assert.match(row.note, /^(refused|ignored):/,
        `docs/cmp-surface.tsv:${row.line} a \`*\` note starts ` +
        `\`refused:\` or \`ignored:\``)

      // The control: the same node with no extra prop must generate,
      // otherwise the case below proves nothing.
      Assert.equal(await generates(sample(row.cmp, {})), undefined,
        `cmp-surface: the sample node for ${row.cmp} does not generate`)

      const err = await generates(sample(row.cmp, { nosuchprop: 1 }))

      if (row.note.startsWith('refused:')) {
        Assert.ok(null != err,
          `docs/cmp-surface.tsv:${row.line} says ${row.cmp} refuses an ` +
          `unknown prop, and it accepted one`)
        Assert.match(err as string, /nosuchprop/,
          `${row.cmp} refused an unknown prop without naming it`)
      }
      else {
        Assert.equal(err, undefined,
          `docs/cmp-surface.tsv:${row.line} says ${row.cmp} ignores an ` +
