...
import { Jostraca, cmpTree, TREE_CMP } from '../'
...
function typeBody(name: string, seen: string[] = []): string {
  Assert.ok(!seen.includes(name),
    'circular type alias: ' + [...seen, name].join(' -> '))
  ...
  for (const f of Fs.readdirSync(CMP_DTS).filter((f) => f.endsWith('.d.ts'))) {
    ...
    Assert.ok(null != alias,
      `${name} is neither an object type nor an alias: ` +
      rhs.split('\n')[0])
    return typeBody((alias as RegExpMatchArray)[1], [...seen, name])
  }
...
function declaredProps(cmp: string): string[] {
...
function readsOf(cmp: string): string[] {
...
function sample(cmp: string, extra: Record<string, any>): any {
  const props = (p: Record<string, any>) => ({ ...p, ...extra })
  const inFile = (node: any) => ({
...
async function generates(node: any): Promise<string | undefined> {
  const { fs } = memfs({ ...SEED })
  try {
    await Jostraca().generate({ fs: () => fs, folder: '/top' }, cmpTree(node))
  }
  catch (err: any) {
    return err.message
  }
...
function diagnostics(): Map<number, string[]> {
  let out = ''
  try {
    out = execFileSync(
      process.execPath, [TSC, '-p', TYPECASE, '--pretty', 'false'],
    ...
    Assert.ok('' !== out.trim(),
      'tsc failed without saying anything: ' + (err.message || err))
  }
  ...
  for (const line of out.split('\n')) {
    ...
    if (-1 !== at && /^\s+\S/.test(line)) {
      const msgs = byLine.get(at) as string[]
...
function markers(): Map<number, string> {
...
describe('cmp-props', () => {
  ...
  test('every-component-declares-and-exports-its-props', () => {
    ...
    Assert.deepEqual(
      onDisk.filter((n) => !INTERNAL.includes(n)),
      Object.keys(TREE_CMP).sort(),
      'src/cmp/ and TREE_CMP disagree about the component set')
    ...
    for (const cmp of onDisk) {
      ...
      Assert.ok(!props.includes(AMBIENT),
        `${cmp}Props declares ${AMBIENT}, which cmp() writes and no ` +
        `caller passes`)
      ...
      Assert.match(entry, new RegExp('\\b' + cmp + 'Props\\b'),
        `jostraca does not export ${cmp}Props`)
    }
  })


  // 2. The props are the ones the code reads.
  test('every-declared-prop-is-read-and-every-read-prop-is-declared', () => {
    for (const cmp of Object.keys(TREE_CMP).sort()) {
      Assert.deepEqual(declaredProps(cmp), readsOf(cmp),
        `${cmp}Props and src/cmp/${cmp}.ts disagree about props`)
    }
  ...
  test('the-published-types-accept-and-refuse-the-right-calls', () => {
    ...
    for (const [line, text] of want) {
      ...
      Assert.ok(0 < got.length,
        `typecase/cases.ts:${line} should not compile, and did`)
      Assert.ok(got.some((m) => m.includes(text)),
        `typecase/cases.ts:${line} was refused without saying ` +
        `"${text}": ${got.join(' | ')}`)
    }

    Assert.deepEqual([...byLine.entries()], [],
      'typecase/cases.ts has errors on lines with no ERR marker')
  })


  // 4. ... and the runtime does what the types say.
  test('an-unknown-prop-is-refused-by-two-and-dropped-by-eight',
    async () => {
      for (const cmp of Object.keys(TREE_CMP).sort()) {
        // The control: the same node with no extra prop must generate,
        // otherwise the case below proves nothing.
        Assert.equal(await generates(sample(cmp, {})), undefined,
          `the sample node for ${cmp} does not generate`)

        const err = await generates(sample(cmp, { nosuchprop: 1 }))

        if (CLOSED.includes(cmp)) {
          Assert.ok(null != err,
            `${cmp} validates a closed prop set and accepted an unknown prop`)
          Assert.match(err as string, /nosuchprop/,
            `${cmp} refused an unknown prop without naming it`)
        }
        else {
          Assert.equal(err, undefined,
            `${cmp} refused an unknown prop; only ` +
